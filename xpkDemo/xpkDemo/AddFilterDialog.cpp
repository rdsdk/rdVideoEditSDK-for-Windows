...
BOOL CAddFilterDialog::OnInitDialog()
{
	...
	filter_count = xpkGetTotalFilter();
...
int CAddFilterDialog::reSetFilter(int ctrl_id)
{

	if(((CButton*)GetDlgItem(ctrl_id))->GetCheck())
		((CButton*)GetDlgItem(ctrl_id))->SetCheck(0);
	return 1;
}
...
void CAddFilterDialog::OnBnClickedLomo()
{
	...
	if(filter_count <= 0 )
	{
		AfxMessageBox("无效的filter");
		return ;
	}
	int filter_id = 11;
	char * filer_name = xpkGetFilterName(filter_id);

	refreshUI(IDC_FLEETING_TIME,filter_id);
}
...
void CAddFilterDialog::OnBnClickedSunsetGlow()
{
	...
	refreshUI(IDC_SUNSET_GLOW,filter_id);
}
...
void CAddFilterDialog::OnBnClickedHawaiianLslands()
{
	...
	int filter_id = 29;
	char * filer_name = xpkGetFilterName(filter_id);
	refreshUI(IDC_HAWAIIAN_LSLANDS,filter_id);
}
...
void CAddFilterDialog::OnBnClickedOvercast()
{
	...
	int filter_id = 28;
	char * filer_name = xpkGetFilterName(filter_id);
	refreshUI(IDC_OVERCAST,filter_id);
}
...
void CAddFilterDialog::OnBnClickedMigratoryBird()
{
	...
	int filter_id = 12;
	char * filer_name = xpkGetFilterName(filter_id);
	refreshUI(IDC_MIGRATORY_BIRD,filter_id);
	
}

void CAddFilterDialog::OnBnClickedCloud()
{
	// TODO: 在此添加控件通知处理程序代码VFT_ACV_CLOUD
	if(filter_count <= 0 )
	{
		AfxMessageBox("无效的filter");
		return ;
...
int CAddFilterDialog::OnClearAllFilter()
{
	...
	reSetFilter(IDC_CLOUD);
	reSetFilter(IDC_MIGRATORY_BIRD);
	reSetFilter(IDC_FLEETING_TIME);
	reSetFilter(IDC_OVERCAST);
	reSetFilter(IDC_HAWAIIAN_LSLANDS);
	reSetFilter(IDC_SUNSET_GLOW);
	reSetFilter(IDC_DUSK);
	return 1;
}